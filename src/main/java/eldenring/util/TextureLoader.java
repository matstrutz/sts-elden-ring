...
public class TextureLoader {
    ...
    public static Texture getTexture(final String filePath, boolean linear) {
        if (textures.get(filePath) == null) {
            try {
                ...
                try
                {
                    return getTexture(EldenRingSTS.resourcePath("missing.png"), false);
                }
                catch (GdxRuntimeException ex) {
                    EldenRingSTS.logger.info("missing.png is missing!");
                    return null;
                }
            }
        }
    ...
    {
        String textureString = EldenRingSTS.resourcePath("cards/" + cardType.name().toLowerCase(Locale.ROOT) + "/" + cardName + ".png");
        ...
        if (!h.exists())
        {
            switch (cardType) {
                case ATTACK:
                    textureString = EldenRingSTS.resourcePath("cards/attack/default.png");
                    break;
                case POWER:
                    textureString = EldenRingSTS.resourcePath("cards/power/default.png");
                    break;
                default:
                    textureString = EldenRingSTS.resourcePath("cards/skill/default.png");
                    break;
            }
        }
    ...
    private static void loadTexture(final String textureString, boolean linearFilter) throws GdxRuntimeException {
        ...
        EldenRingSTS.logger.info("Loaded texture " + textureString);
        textures.put(textureString, texture);
    }

    public static Texture getPowerTexture(final String powerName)
    {
        String textureString = EldenRingSTS.powerPath(powerName + ".png");
        return getTexture(textureString);
    }
    public static Texture getHiDefPowerTexture(final String powerName)
    {
        String textureString = EldenRingSTS.powerPath("large/" + powerName + ".png");
